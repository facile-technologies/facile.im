...
export default function ContactCustomization() {
  ...
  const contactCustomization = useSelector(
    (state) => state.sosprofile.ContactCustomization
  );
  const loading = useSelector(selectSOSLoading);

  // Initialize toggle from Redux
  useEffect(() => {
  ...
  // Get the current color for the active tab
  ...
  // Apply a color to the active tab immediately
  ...
  const handleResetColors = () => {
    dispatch(setContactCustomizationBackgroundColor(DEFAULT_COLORS.backgroundColor));
    dispatch(setContactCustomizationheadertextColor(DEFAULT_COLORS.headerTextColor));
    dispatch(setContactCustomizationTextColor(DEFAULT_COLORS.bodyTextColor));
  };

  const handleSaveCustomization = () => {
    const customizationData = {
      title_color: contactCustomization?.title,
    ...
    dispatch(updateContcatCustomizations(customizationData))
      .then(() => dispatch(fetchSOSProfile()))
      .catch(() => showToast("error", "Failed to update contact customization!"));
  };
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      ...
      <h3 className="dark:text-white text-black text-[16px] font-semibold mt-2">
      ...
      <div className="flex flex-col dark:bg-[#303030] bg-[#F5F5F5] rounded-2xl p-4 border border-[#C0C0C017] w-full gap-3">
        <div>
          <h3 className="dark:text-white text-black text-[15px] font-semibold">Title</h3>
