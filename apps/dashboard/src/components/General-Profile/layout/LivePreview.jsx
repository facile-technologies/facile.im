...
export default function LivePreview() {
  ...
  const linkIconStyle = useSelector(
    (state) => state.profile.linkCustomization?.icon_styled,
  );
      ...
      <div className="grid grid-cols-2 gap-4 py-2 w-full">
        {customLinks
          ...
          .map((link, idx) => {
            ...
            // 🛡️ Strict fallback: Use icon, then profile logo, then project logo.
            // Never use thumbnail as icon if it's identical or if logo is missing.
            const overlayIcon =
  ...
  const renderCustomCarousel = () => (
    <div className="relative w-full flex items-center py-6">
      {showArrows && (
        <button
          onClick={handlePrev}
          className="absolute left-1 z-10 !bg-transparent !text-white drop-shadow-md text-[35px] border-none outline-none p-0 flex items-center justify-center translate-y-[-10px]"
        >
          &#8249;
        </button>
      )}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Overlay for carousel items (Top Right) */}
      ...
      {showArrows && (
        <button
          onClick={handleNext}
          className="absolute right-1 z-10 !bg-transparent !text-white drop-shadow-md text-[35px] border-none outline-none p-0 flex items-center justify-center translate-y-[-10px]"
        >
          &#8250;
        </button>
      )}
  ...
  const renderCasual = () => (
    <div className="relative w-full flex items-center py-4">
      ...
      <div
        ...
        className="flex gap-0.5 overflow-x-auto scroll-smooth no-scrollbar w-full px-1"
      >
        {platformLinks
          .filter((link) => link.isVisible)
          .map((link) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="min-w-[68px] flex flex-col items-center justify-center"
            >
              <img
    ...
    <div className="flex flex-col gap-4 w-auto mx-auto p-4 shrink-0">
      ...
      <div className="relative mx-auto bg-transparent group flex flex-col gap-4">
        ...
        <div className="relative">
          {/* Physical Buttons - Decorative (Decorative) */}
          ...
          bg-[#7B7B7B] border-4 border-white dark:border-[#2B2B2B] flex items-center justify-center text-xs`}
                  ...
                  <div className="w-full mb-2 dark:bg-[#2B2B2B] rounded-[15px] p-4 flex flex-col gap-2">
                    ...
                    {(emailLayout || "left") === "left" ? (
                      <div className="flex flex-row gap-0 items-center justify-center p-4">
                        <input
                          placeholder={
                            emailFields?.find((f) => f.is_enabled)
                ...
                {saveContact ? (
                  <div className=" rounded-[15px] p-4 text-center w-full mb-6">
                    <a
                      type=" button"
                      className="bg-[#4F2E86] h-[51px] items-center flex justify-center text-white text-sm px-4 py-2"
                      style={{
                                ...
                                >
                                  ...
                                  <div className="absolute bottom-2 left-0 right-0 flex justify-between items-center  p-2 rounded-b-lg">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <a
                                        type="button"
                                        className=" text-sm px-4 py-2 rounded-3xl  transition"
                                        style={{
                                          backgroundColor:
                                            buyBtnBgColor || "#000",
