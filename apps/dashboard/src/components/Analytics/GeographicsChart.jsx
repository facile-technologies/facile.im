...
function buildSegments(geographic = []) {
  const sorted = [...geographic].sort((a, b) => b.orders - a.orders).slice(0, RADII.length);
          ...
          <>
            <div className="relative" style={{ width: 220, height: 220 }}>
              ...
              >
                ...
                <span className="text-xs text-gray-400 mt-1 tracking-wide">orders</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-10 gap-y-4 mt-6 w-full max-w-[260px]">
              {segments.map((seg) => (
                <div key={seg.name} className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400 font-medium">{seg.name}</span>
                </div>
              ))}
            </div>
          </>
