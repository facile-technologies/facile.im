...
export const mapProfileViewResponseToState = (data) => {
  ...
  const customLinks = (data.customLinks || []).map((cl) => ({
    ...
    isVisible: cl.is_visible ?? true, // ✅ FIX
  }));
  ...
  return {
    ...
    platformLinks: (data.links || []).map((link) => {
      ...
      return {
        ...
        isVisible: link.is_visible ?? true, // ✅ FIX
      };
    }),
    customLinks,
    medias,
    ...
    newsletterButtonRadius: contact.button_corner_radius || 19,
